...
//        System.out.println(uploadPath+"hh");
...
//                        System.out.println(filePath);
                ...
                itemBean.setItemIfo(dataMap.get("itemIfo"));
                itemBean.setItemClass(dataMap.get("itemClass"));
...
//                System.out.println(itemBean.getFilename()+itemBean.getItemId()+itemBean.getItemName());
        		if(b==0)//数据库id不存在，插入
        		{
                    ...
                    UserBean user=(UserBean)request.getSession().getAttribute(type);
                    ActionUtils.WriteDownActions(type,user.getUserName(),user.getIp(),"添加商品");
...
//        			response.sendRedirect("ListItemServlet2");
        		...
        		{
        			request.setAttribute("message", "该商品编号已存在");
